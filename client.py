    ...
    try:
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if save_img & len(det) > 0:
                    response = {"results": "People Detected!"}
                    ...
                    response = {"results": "No People Detected!"}
        ...
        response = {"results": "Error Occured!"}
...
def people_detect_url(url):

    save_dir = f"store_img/"

    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    img_data = requests.get(url).content
    file_name = save_dir + str(url.split("?")[0].split("/")[-1])
